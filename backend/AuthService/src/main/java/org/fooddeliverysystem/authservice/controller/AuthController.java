...
import org.fooddeliverysystem.authservice.dto.authdtos.*;
...
@AllArgsConstructor
@Tag(description = "Authentication APIs", name = "Auth Controller")
    ...
    public ResponseEntity<TokenResponse> login(@Valid @RequestBody LoginRequest request, HttpServletResponse response) {
        ...
        return ResponseEntity.ok()
                ...
                .body(TokenResponse.bearerWithUser(accessToken, refreshToken, 900,
                        new UserDto(user.getName(), user.getEmail(), user.isEnabled(), user.getImage(), user.getCreatedAt(), user.getUpdatedAt())));
    }
